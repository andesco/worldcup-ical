...
describe("feed", () => {
  it("selects only qualifying fixtures with reasons", () => {
    const sel = selectMatches(fixtures, oddsMap, { teams: new Set(["USA"]), rank: 8, competitive: 10 });
  ...
  it("an empty selection still yields a valid empty calendar", () => {
    const ics = buildFeed(fixtures, oddsMap, { teams: new Set(), rank: null, competitive: null });
    expect(ics).toContain("BEGIN:VCALENDAR");
